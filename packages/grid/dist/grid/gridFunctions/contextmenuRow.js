...
export function contextmenuRow(ctx, event, cell, row, column, celno, colType, cellType, attribute, rowData) {
    ...
    const generateCopyPasteData = (attributes, onlyCurrentEntity, overRideCurrentEntity = null) => {
        const TableOuterTop = `
            <html>
                <body>
                <style>
    ...
    const copyData = (attributes, onlyCurrentEntity, overRideCurrentEntity = null) => {
    ...
    const pasteIntoCells = (attribute, data) => {
        const datasource = ctx.gridInterface.getDatasource();
        const attConfig = ctx.gridInterface.__getGridConfig().__attributes;
        const cellConfig = attConfig[attribute];
    ...
    const summaryTemplate = () => {
        ...
        return html ` <div class="simple-html-grid-menu-section">Summary:</div>
            ...
            <div
                ...
                @click=${async () => {
            await navigator.clipboard.writeText(valueFormater.fromSource(selectedRows.length, 'number', attribute, false));
            removeContextMenu(ctx);
        }}
            >
    ...
    const filterTemplate = () => {
        ...
        const applyFilter = (filterArgument) => {
...
//# sourceMappingURL=contextmenuRow.js.map