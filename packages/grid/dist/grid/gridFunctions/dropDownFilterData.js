 ...
 * @returns
 */
export function dropDownFilterData(ctx, attribute, availableOnly, searchInput) {
        ...
        oldFilter?.filterArguments.forEach((f) => {
            if (f.attribute === attribute) {
                if (Array.isArray(f.value)) {
                    if (f.operator === 'IN') {
                        dataFilterSet = new Set(f.value);
                        selectAll = false;
                    }
                    if (f.operator === 'NOT_IN') {
                        const tempSet = new Set(f.value);
...
//# sourceMappingURL=dropDownFilterData.js.map