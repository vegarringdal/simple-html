 ...
 */
export function rebuildHeaderColumns(ctx) {
    const config = ctx.gridInterface.__getGridConfig();
    /**
     * helper to generate cols and rows elements
     */
    const addColumns = (parent, columns, maxColumns, coltype) => {
        ...
        if (!parent) {
            ...
            for (let i = 0; i < columnsNeeded; i++) {
                ...
                resizeElement.onmousedown = (event) => {
                    ...
                    const refID = parseInt(event.target.parentElement.getAttribute('refID'), 10);
                    ...
                    const mousemove = (event) => {
                        if (col) {
                            ...
                            verticalScrollHandler(ctx, getElementByClassName(ctx.element, 'simple-html-grid-body-view-pinned-middle').scrollTop);
    ...
    addColumns(getElementByClassName(ctx.element, 'simple-html-grid-header-row-container-pinned-left'), config.columnsPinnedLeft, 0, LEFT_PINNED_COLTYPE);
    ...
    addColumns(getElementByClassName(ctx.element, 'simple-html-grid-header-row-container-pinned-middle'), config.columnsCenter, middleWidth / 50, MIDDLE_PINNED_COLTYPE);
    addColumns(getElementByClassName(ctx.element, 'simple-html-grid-header-row-container-pinned-right'), config.columnsPinnedRight, 0, RIGH_PINNED_COLTYPE);
    /**
...
//# sourceMappingURL=rebuildHeaderColumns.js.map