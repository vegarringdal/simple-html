...
export function addScrollEventListeners(ctx) {
    ...
    // helper
    function setScrollLeft(element, left) {
     ...
     */
    ctx.element.addEventListener('wheel', (event) => {
        if (event.shiftKey && event.ctrlKey) {
            event.preventDefault();
            const el = getElementByClassName(ctx.element, 'simple-html-grid-middle-scroller');
            const movement = el.scrollLeft - event.wheelDeltaY;
            ...
            const movement = el.scrollTop - event.wheelDeltaY;
            setScrollTop(getElementByClassName(ctx.element, 'simple-html-grid-body-scroller'), movement);
        }
    }, { passive: false });
    let lastY = 0;
    let lastX = 0;
    ctx.element.addEventListener('touchstart', (event) => {
     ...
     *wheel event, only way to get it unless we disable pointer events
     */
    ctx.element.addEventListener('touchmove', (event) => {
...
//# sourceMappingURL=addScrollEventListeners.js.map