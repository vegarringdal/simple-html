 ...
 * @param height
 */
export function updateHorizontalScrollWidth(ctx) {
    const middlec = getElementByClassName(ctx.element, 'simple-html-grid-middle-scroller-body');
    middlec.style.width = asPx(ctx.gridInterface
        .__getGridConfig()
    ...
    middlex.style.width = asPx(ctx.gridInterface
        .__getGridConfig()
        .columnsCenter.map((e) => e.width)
        .reduce((prev, cur) => prev + cur, 0));
    const middleh = getElementByClassName(ctx.element, 'simple-html-grid-header-row-container-pinned-middle');
    middleh.style.width = asPx(ctx.gridInterface
        .__getGridConfig()
        .columnsCenter.map((e) => e.width)
        .reduce((prev, cur) => prev + cur, 0));
}
//# sourceMappingURL=updateHorizontalScrollWidth.js.map