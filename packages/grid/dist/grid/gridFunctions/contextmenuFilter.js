...
export function contextmenuFilter(ctx, event, cell, _row, _column, _celno, _colType, _cellType, attribute, _rowData) {
    ...
    const searchTemplate = (reRender) => {
        ...
        const runFilterClick = () => {
            ...
            if (intersection.length < getContext().data.dataFilterSet.size) {
                ...
                filterCallback(ctx, null, cellConfig, intersection.length ? intersection : null, getContext().searchInput ? getContext().searchInput : null, !!intersection.length);
            }
            else {
                filterCallback(ctx, null, cellConfig, getContext().data.dataFilterSet.size ? Array.from(getContext().data.dataFilterSet) : null, getContext().searchInput ? getContext().searchInput : null, false);
            }
            removeContextMenu(ctx);
        };
        /**
         ...
         */
        const filterValues = () => {
            const filterValueClick = (rowData) => {
            ...
            return Array.from(getContext().data.dataFilterSetFull).map((rowData) => {
        ...
        const availableCheckbox = () => {
            ...
            return html ` <div style="padding:2px">
                <input
                    ...
                    @click=${(e) => handleEvent(e)}
                /><label style="padding:2px" @click=${(e) => handleEvent(e)}>Filter Available</label>
            </div>`;
        };
        /**
    ...
    const innerRender = () => {
        ...
        const filterTemplate = () => {
            ...
            const updateFilter = () => {
                filterCallback(ctx, cellConfig.currentFilterValue, cellConfig);
        ...
        render(html `<div class="simple-html-grid-menu">
...
//# sourceMappingURL=contextmenuFilter.js.map