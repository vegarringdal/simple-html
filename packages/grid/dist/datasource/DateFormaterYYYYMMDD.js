/**
 * default dateformater - YYYY-MM-DD
 */
export class DateFormaterYYYYMMDD {
    /**
     * Takes value and return string
     * @param value
     */
    static fromSource(value) {
        let returnValue = value;
        if (returnValue === null || returnValue === undefined || returnValue === '') {
            return '';
        }
        returnValue = new Date(returnValue).toDateString();
        // clear if not valid date
        if (returnValue === 'Invalid Date') {
            returnValue = '';
        }
        if (returnValue) {
            // get our formating
            const year = new Date(value).getFullYear();
            let month = (new Date(value).getMonth() + 1).toString();
            if (month.length === 1) {
                month = `0${month}`;
            }
            let day = new Date(value).getDate().toString();
            if (day.length === 1) {
                day = `0${day}`;
            }
            returnValue = `${year}-${month}-${day}`;
        }
        return returnValue;
    }
    /**
     *
     * @param value Takes string and returns date
     */
    static toSource(value) {
        let returnValue = value;
        if (returnValue === null || returnValue === undefined) {
            return returnValue;
        }
        if (returnValue < 1) {
            return null;
        }
        if (typeof returnValue !== 'string') {
            return null;
        }
        const x = returnValue.split('-');
        // 2019-01-01
        if (!x[1]) {
            // by default, use current month
            x[1] = new Date().getMonth() + 1;
        }
        if (!x[2]) {
            // by default, use current day
            x[2] = new Date().getDate();
        }
        returnValue = new Date(x[0], parseInt(x[1], 10) - 1, parseInt(x[2], 10), new Date().getHours(), new Date().getMinutes(), new Date().getSeconds(), new Date().getMilliseconds());
        if (returnValue && typeof returnValue === 'object' && returnValue.toString() === 'Invalid Date') {
            returnValue = '';
        }
        return returnValue;
    }
    /**
     * will be used in filters, you might want other logic here
     * @param value
     * @returns
     */
    static toFilter(value) {
        let returnValue = value;
        if (returnValue === null || returnValue === undefined) {
            return returnValue;
        }
        if (returnValue < 1) {
            return null;
        }
        if (typeof returnValue !== 'string') {
            return null;
        }
        const x = returnValue.split('-');
        // 2019-01-01
        if (!x[1]) {
            // by default, use current month
            x[1] = new Date().getMonth() + 1;
        }
        if (!x[2]) {
            // by default, use current day
            x[2] = new Date().getDate();
        }
        returnValue = new Date(x[0], parseInt(x[1], 10) - 1, parseInt(x[2], 10), 0, 0, 0, 0);
        if (returnValue && typeof returnValue === 'object' && returnValue.toString() === 'Invalid Date') {
            returnValue = '';
        }
        return returnValue;
    }
    static fromSourceDisplay(value) {
        return DateFormaterYYYYMMDD.fromSource(value);
    }
    static fromSourceGrouping(value) {
        return DateFormaterYYYYMMDD.fromSourceDisplay(value);
    }
    static placeholder() {
        return 'YYYY-MM-DD';
    }
}
//# sourceMappingURL=DateFormaterYYYYMMDD.js.map