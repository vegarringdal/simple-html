...
export class Filter {
    ...
    orStatement(rowData, ObjFilter, ds) {
        if (Array.isArray(ObjFilter.filterArguments)) {
            for (let i = 0; i < ObjFilter.filterArguments.length; i++) {
                ...
                if (filter.logicalOperator === 'NONE' ||
                    ...
                    if (filter.operator === 'IN' || filter.operator === 'NOT_IN') {
                        ...
                        if (!Array.isArray(filter.value)) {
                            ...
                            values = filter.value.toUpperCase().split('\n');
                            filter.value = values;
                        }
                        else {
    ...
    andStatement(rowData, ObjFilter, ds) {
        if (Array.isArray(ObjFilter.filterArguments)) {
            for (let i = 0; i < ObjFilter.filterArguments.length; i++) {
                const filter = ObjFilter.filterArguments[i];
                if (filter.logicalOperator === 'AND') {
