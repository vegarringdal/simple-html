export function objectFilter(rowData, filter) {
    let result = true;
    // vars
    let rowValue;
    let filterValue;
    ...
    switch (type) {
        case 'date':
            try {
                ...
                rowValue = new Date(new Date(rowValue).getFullYear(), new Date(rowValue).getMonth(), new Date(rowValue).getDate(), new Date(rowValue).getHours(), new Date(rowValue).getMinutes(), new Date(rowValue).getSeconds(), 0);
            ...
            try {
                // we need to reset date so they use same time/timezone
                filterValue = new Date(new Date(filter.value).getFullYear(), new Date(filter.value).getMonth(), new Date(filter.value).getDate(), new Date(filter.value).getHours(), new Date(filter.value).getMinutes(), new Date(filter.value).getSeconds(), 0);
        ...
        case 'text': {
            ...
            switch (filterOperator) {
                case 'BEGIN_WITH':
                    ...
                    filterValue = new RegExp(`^${updateRegex(filterValue)}.*`, 'gim');
                    break;
                case 'CONTAINS':
                    newFilterOperator = 'REGEX';
                    filterValue = new RegExp(`.*${updateRegex(filterValue)}.*`, 'gim');
                ...
                case 'NOT_EQUAL_TO': {
                    ...
                    filterValue = new RegExp(`${start}${updateRegex(filterValue)}${end}`, 'gim');
                    break;
                }
                default:
